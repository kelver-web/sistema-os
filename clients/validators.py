...
def validate_document(value):
    ...
    if len(numbers) == 11:
        if not CPF().validate(numbers):
            raise serializers.ValidationError('CPF inválido, verifique os dados informados.')
    elif len(numbers) == 14:
        if not CNPJ().validate(numbers):
            raise serializers.ValidationError('CNPJ inválido, verifique os dados informados.')
    else:
