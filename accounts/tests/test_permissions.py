...
def admin_user(db):
    return User.objects.create_user(
        username="admin_user", email="admin_user@teste",
        password="testpass", role=User.Role.ADMIN
    )
    
@pytest.fixture
def tech_user(db):
    return User.objects.create_user(
        username="tech_user", email="tech_user@teste",
        password="testpass", role=User.Role.TECH
    )
    
@pytest.fixture
def attendant_user(db):
    return User.objects.create_user(
        username="attendant_user", email="attendant_user@teste",
        password="testpass", role=User.Role.ATTENDANT
    )
